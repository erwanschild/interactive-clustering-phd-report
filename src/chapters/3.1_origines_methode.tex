...
\label{section:3.1-INTUITIONS-ORIGINES}
	...
	\label{section:3.1.1-INTUITIONS-ORIGINES-NON-SUPERVISEES}
		...
		\begin{itemize}
			...
			Plusieurs liens de similarité peuvent être implémentés (\textit{le lien \texttt{single} fusionnant les deux \textit{cluster} ayant les frontières les plus proches, le lien \texttt{complete} fusionnant les deux \textit{cluster} ayant les frontières opposées les plus proches, le lien \texttt{average} fusionnant les deux \textit{cluster} ayant les barycentres les plus proches et le lien \texttt{ward} fusionnant les deux \textit{cluster} qui donneront le prochain \textit{cluster} le plus compact}) ;
			% Spectral.
			...
			Cette approche permet d'obtenir des \textit{cluster} aux formes complexes ;
			% DBScan.
			...
			Cette approche permet de découvrir des \textit{cluster} aux formes complexes, si leur densité est suffisante.
			\item ...
		\end{itemize}
	...
	\label{section:3.1.2-INTUITIONS-ORIGINES-SEMI-SUPERVISEES}
		...
		\begin{itemize}
			...
			\item les \textbf{contraintes sur les \textit{cluster}} : ces contraintes peuvent concerner le nombre de \textit{cluster} à trouver, leur taille minimale ou maximale, la distance minimale de séparation de leurs frontières, ...
		\end{itemize}
		...
		Comme cet expert n'a a priori pas de connaissances techniques ou analytiques, ce dernier aurait du mal à manipuler des contraintes sur les \textit{cluster}.
		...
		\begin{itemize}
			...
			dans cette version, les données liées par des contraintes \texttt{MUST-LINK} sont d'abord fusionnées, puis le processus agglomératif commence en prenant garde de ne pas fusionner des \textit{cluster} ayant des contraintes \texttt{CANNOT-LINK} entre eux.
			Cette adaptation est très simple à mettre en oeuvre car il suffit d'adapter le calcul de distance entre \textit{cluster} ;
			% Spectral.
			...
			dans cette version, la densité des données ainsi que les contraintes \texttt{CANNOT-LINK} sont utilisées pour identifier des \textit{cluster} locaux qui seront ensuite fusionnés à l'aide des contraintes \texttt{MUST-LINK}
