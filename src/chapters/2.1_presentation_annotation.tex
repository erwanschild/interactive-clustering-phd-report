...
\label{section:2.1-PRESENTATION-ANNOTATION}
	...
	\label{section:2.1.1-PRESENTATION-ANNOTATION-DEFINITION}
		...
		\label{section:2.1.1.A-PRESENTATION-ANNOTATION-DEFINITION-MACHINE-LEARNING}
			...
			Cette apprentissage peut être \textit{supervisé} (l'interprétation de les exemples est fournie par un humain), \textit{non-supervisé} (la machine déduit l'interprétation des données sans intervention humaine) ou \textit{semi-supervisé} (mélange des deux approches).
			
			% Applications.
			L'apprentissage automatique, très populaire en ce moment, permet ainsi d'automatiser l'analyse et la manipulation de certains phénomènes complexes tels que le langage, l'observation visuelle, la détection d'anomalies, le traitement acoustique, ...
		...
		\label{section:2.1.1.B-PRESENTATION-ANNOTATION-DEFINITION-BASE-APPRENTISSAGE}
			...
			% References.
			\begin{leftBarInformation}
				Nous discuterons davantage de cette notion de représentativité dans la (voir \textsc{Section~\ref{section:2.3.1.A-DEFIS-ANNOTATION-ASPECT-DONNEES-REPRESENTATIVITE}}).
	...
	\label{section:2.1.2-PRESENTATION-ANNOTATION-EXEMPLES}
		...
		\label{section:2.1.2.A-PRESENTATION-ANNOTATION-EXEMPLES-REGRESSION}
			...
			% Autre cas d'usage similaire: Régression linéaire dans d'autres domaines.
			\begin{leftBarInformation}
		...
		\label{section:2.1.2.D-PRESENTATION-ANNOTATION-EXEMPLES-TRANSCRIPTION}
			...
			Mais dans le cadre de notre exemple sur le thème des bandes dessinées, peu d'entre elles disposent d'une lecture audio.
			...
			Mais pour entraîner de tels modèles, il est nécessaire d'avoir une base d'apprentissage contenant des exemples d'audios prononcés par chacun des personnages (ici: les doubleurs de l'adaptation télévisée) avec la transcription de leur paroles pour chaque audio.
			...
			\begin{leftBarInformation}
				...
				Bien entendu, la conception et l’enchaînement de ces différents modèles est une tâche plutôt complexe, et chaque tâche de \textit{Machine Learning} demande ses propres données annotées pour construire une base d'apprentissage.
			\end{leftBarInformation}
