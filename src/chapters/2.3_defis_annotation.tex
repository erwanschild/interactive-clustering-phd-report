...
\label{section:2.3-DEFIS-ANNOTATION}
	...
	\label{section:2.3.3-DEFIS-ANNOTATION-ASPECT-HUMAIN}
		...
		\label{section:2.3.3.B-DEFIS-ANNOTATION-ASPECT-HUMAIN-INTRA-ANNOTATEURS}
			...
			Malgré la conception d'un guide et la rédaction de règles, malgré les sessions de revues dédiées à affiner ce guide et malgré les diverses pistes mises en oeuvre pour assister l'annotation, nous pouvons constater qu'un annotateur peut tout de même manifester des variations de comportement.
			Ces différences peuvent s'apparenter à des erreurs d'innattention, des oublis de consignes, ou encore des fluctuations passagères.
			
			Pour expliquer cela, nous pouvons nous baser sur la théorie de la \textbf{régulation de la charge de travail} (\cite{sperandio:1978:regulation-working-methods}, \cite{sperandio:1987:ergonomie-travail-mental}) : peu importe la tâche, l'opérateur aura tendance à percevoir ses capacités comme minime par rapport à la charge de travail à réaliser ; celui-ci cherchera donc un moyen d'adapter son travail afin de diminuer la charge de travail perçue et donc de la rendre acceptable ; cette adaptation explique ainsi les différences de comportement.
			%
			\begin{leftBarExamples}
				Une bonne illustration est celle de la \textit{veille du week-end} :
				la tâche à souvent tendance à être plus complexe à ce moment-là, alors que celle-ci n'a pourtant pas changer.
				Les opérateurs adaptent toutefois leur travail pour diminuer la carge de travail perçue.
				Conclusion : la pause café est un peu plus longue, et une partie du travail est finalement reportée à la semaine suivante.
			\end{leftBarExamples}
			
			%
			\todo[inline]{quelques solutions: simplification, gamification}
			\begin{leftBarIdea}
