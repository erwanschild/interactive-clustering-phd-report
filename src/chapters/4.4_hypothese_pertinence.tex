...
\label{section:4.4-HYPOTHESE-PERTINENCE}
	...
	\label{section:4.4.1-ETUDE-PERTINENCE-VERIFICATION-MANUELLE}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				...
				%
				\caption{\textit{Description en pseudo-code du protocole expérimental de l'étude de vérification manuelle non assistée de la valeur métier d'une base d'apprentissage.}}
	...
	\label{section:4.4.2-ETUDE-PERTINENCE-PATTERNS-LINGUISTIQUES}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				...
				\ForEach{jeux de données à tester}{
					...
					\textbf{vectorisation}: transformer les données en vecteurs avec \texttt{vect.tfidf} \;
					\textbf{clustering initial}: regrouper les données par similarité avec \texttt{clust.kmeans.cop} \;
					\textbf{analyse linguistique}: déterminer le vocabulaire caractéristique de chaque \textit{cluster} avec la \texttt{FMC} \;
				...
				\KwResult{discussion sur la complexité de la tâche et sur l'évolution de l'exploitabilité}
				%
				\caption{\textit{Description en pseudo-code du protocole expérimental de l'étude des patterns linguistiques pertinents pour vérifier la valeur métier d'une base d'apprentissage.}}
		...
		\subsubsection{Discussion}
			...
			% Avantage 1: Unitlité de la FMC pour les clusters exploitables.
			...
			% Avantage 2: Unitlité de la FMC pour les clusters partiellement exploitables.
			...
			% Avantage 3: Unitlité de la FMC pour les clusters non exploitables.
	...
	\label{section:4.4.3-ETUDE-PERTINENCE-RESUME-AUTOMATIQUE}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				...
				\ForEach{jeux de données à tester}{
					...
					\textbf{vectorisation}: transformer les données en vecteurs avec \texttt{vect.tfidf} \;
					\textbf{clustering initial}: regrouper les données par similarité avec \texttt{clust.kmeans.cop} \;
					\textbf{résumé automatique}: utiliser un \texttt{LLM} pour synthétiser les thématiques de chaque \textit{cluster} \;
					\textbf{évaluation assistée}: juger de l'exploitabilité de chaque \textit{cluster} \;
					\textbf{labellisation assistée}: nommer chaque \textit{cluster} exploitable \;
					\Repeat{annotation de toutes les contraintes possibles}{
						\textbf{échantillonnage}: sélectionner de nouvelles contraintes à annoter \;
				...
				\KwResult{discussion sur la complexité de la tâche et sur l'évolution de l'exploitabilité}
				%
				\caption{\textit{Description en pseudo-code du protocole expérimental de l'étude d'un résumé automatique des \textit{clusters} à l'aide d'un large modèle de langage pour vérifier la valeur métier d'une base d'apprentissage.}}
		...
		\subsubsection{Résultats obtenus}
			...
			De manière général, la majorité des résumés automatiques permettent d'identifier sans ambiguïté les mêmes thématiques qu'une vérification manuelle.
			\begin{itemize}
				\item En ce qui concerne les itérations où le \textit{clustering} atteint la vérité terrain, tous les résumés permettent d'identifier les thématiques présentes ($190$ \textit{clusters} concernés) ;
				\item En ce qui concerne les autres itérations (dont le \textit{clustering} ne correspond pas encore à la vérité terrain), les résumés automatiques permettent d'identifier les mêmes thématiques qu'une vérification manuelle dans $86$\% des cas ($331$ cas sur $386$ \textit{clusters}).
			\end{itemize}
		...
		\subsubsection{Discussion}
			...
			% Limite 1: Présence de quelques cas aberrantes.
			...
			\begin{itemize}
				...
				\item à l'inverse, les données aberrantes ou isolées d'un \textit{cluster} peuvent influencer le résumé : cela peut donner l'illusion que plusieurs thématiques sont présentes alors qu'une seule ne l'est réellement ;
			...
			Néanmoins, nous notons plusieurs zones d'ombres tant sur l'utilisation des modèles (infrastructure technique lourde, confidentialité des données, ...) que sur l'exploitation de leurs résultats (hallucinations, ambiguïtés, ...).
		
		
	%%%
	%%% Subsection 4.4.4: Mise en commun des stratégie d'évaluation de la pertinence métier d'un résultat de \textit{clustering} interactif
	%%%
	\subsection{Mise en commun des stratégie d'évaluation de la pertinence métier d'un résultat de \textit{clustering} interactif}
	\label{section:4.4.4-ETUDE-PERTINENCE-MISE-EN-COMMUN}
		...
		\begin{leftBarSummary}
			...
			\begin{itemize}
				\item[\itemok] La tâche de vérification et de labellisation de résultats de \textit{clustering} est plutôt complexe et fastidieuse si elle n'est pas assistée : il est donc conseillé de faire intervenir plusieurs experts afin de consolider leurs analyses ;
				...
				\item[\itemok] Cette approche automatisée n'étant pas infaillible, il est recommandé de tout de même vérifier manuellement le contenu des \textit{clusters} avant d'arrêter le projet d'annotation : pour faciliter cette revue, il est possible de réaliser une analyse linguistique grâce à la \texttt{FMC} pour identifier les termes caractéristique de chaque \textit{cluster} et les mettre en avant dans le texte (cf. \textsc{Section~\ref{section:4.4.2-ETUDE-PERTINENCE-PATTERNS-LINGUISTIQUES}}).
			\end{itemize}
		\end{leftBarSummary}
		
		% Transition: Vers Rentabilité.
		Pour compléter l'étude que nous venons de réaliser, il peut être intéressant de définir un cas d'arrêt des itérations du \textit{clustering} interactif afin de pouvoir prédire quand arrêter l'annotation et demander à l'expert d'évaluer la pertinence de la base d'apprentissage obtenue.
		Nous étudierons cette aspect dans la prochaine section (cf. hypothèse de rentabilité en \textsc{Section~\ref{section:4.5-HYPOTHESE-RENTABILITE}}).
