...
\label{section:4.6-HYPOTHESE-ROBUSTESSE}
	...
	\end{itemize}
	%%%%\begin{itemize}
	%%%%	\item une étude du \textbf{score inter-annotateurs} obtenu lors d'une expérience d’annotation de contraintes avec plusieurs opérateurs en situation réelle (cf. \textsc{Section~\ref{section:4.6.3-ETUDE-ROBUSTESSE-SCORE-INTER-ANNOTATEURS}}) ;
	%%%%	\item une étude de l'intérêt de la correction des incohérences d'annotation (cf. \textsc{Section~\ref{section:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-ERREURS}}) ;
	%%%%	\item et une simulation de l'impact des incohérences d'annotation sur les performances de la méthode (cf. \textsc{Section~\ref{section:4.6.2-ETUDE-ROBUSTESSE-SIMULATION-IMPACT-ERREURS}}).
	%%%%\end{itemize}
	...
	\label{section:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-ERREURS}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				...
				\ForEach{arrangement de taux d'erreur à tester}{
					...
					\Repeat{annotation de toutes les contraintes possibles}{
						\textbf{échantillonnage}: sélectionner de nouvelles contraintes avec \texttt{samp.closest.diff} \;
						\textbf{échantillonnage d'erreurs}: définir les contraintes qui seront erronées \;
						\textbf{simulation d'annotation}: caractériser les contraintes en utilisant la vérité terrain \;
						\If{absence de correction des conflits d'annotation}{
							\textbf{intégration naïve}: ajouter les nouvelles contraintes au gestionnaire de contraintes, ignorer les contraintes incohérentes avec la base existante \;
						}
						\ElseIf{détection et correction des conflits d'annotation}{
							\textbf{intégration avec correction}: ajouter les nouvelles contraintes au gestionnaire de contraintes, annuler les annotations erronées qui sont détectées par le gestionnaire \;
						}
				...
				\label{algorithm:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-ERREURS}
			\end{algorithm}
			...
			\begin{itemize}
				...
				\item Les contraintes à insérer en erreur sont tirées aléatoirement parmi le lot de contraintes qui aurait été échantillonné au cours d'une tentative sans erreur : ainsi, nous pouvons comparer toutes ces simulations itération par itération car elles partagent la même base de contraintes (aux valeurs de \texttt{MUST-LINK} et \texttt{CANNOT-LINK} près) ;
			\end{itemize}
			
			Puisque nous introduisons des erreurs d'annotations, des conflits vont apparaître dans le gestionnaire de contraintes.
			Comme énoncé plus haut, nous utilisons deux approches pour les traiter :
			\begin{itemize}
				\item %todo
				\item %todo
			\end{itemize}
			
			Ainsi, il y a donc $11$ taux d'erreurs différents à simuler, chacun suivant $2$ approches de travail (avec et sans correction), et chacune de ces simulations d'erreurs seront répétées $10$ fois sur chaque tentative complète de la méthode pour limiter contrer les aléas statistiques des tirages de contraintes erronées, ce qui représente $220$ simulation par tentatives.
			...
			\begin{leftBarWarning}
				Pour nous permettre de comparer facilement les résultats, ces erreurs sont choisies aléatoirement parmi l'échantillonnage de contraintes qu'il y aurait eu lieu si n'y avait pas de contraintes.
			\end{leftBarWarning}

			contraint
			ajoutons au cours des itérations, nous simulons des erreurs d'annotations représentant entre $0$ et $50$\% du lot de contraintes.
		...
		\subsubsection{Résultats obtenus}
			...
			La \textsc{Figure~\ref{figure:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-ERREURS}} ...
			
			\begin{figure}[!htb]
				\centering
				\includegraphics[width=0.95\textwidth]{figures/etude-erreur-simulation-impact-closest}
				\caption{
					Évolutions de la moyenne de la \texttt{v-measure} entre un résultat obtenu et la vérité terrain en fonction du nombre de contraintes annotées au cours des itérations du \textit{clustering} interactif et en prenant en compte un taux d'erreurs d'annotations compris entre $0$\% et $50$\%.
					\textbf{(1)} représente l'évolution de la performance si les incohérences ne sont pas corrigées, \textbf{(2)} représente la même évolution avec correction des incohérences dès qu'elles sont détectées.
				}
	...
	\label{section:4.6.2-ETUDE-ROBUSTESSE-SIMULATION-IMPACT-ERREURS}
		...
		\subsubsection{Discussion}
			...
			% Remaques expérience utilisateur.
			\todo[inline]{A REDIGER: prédiction retard}
