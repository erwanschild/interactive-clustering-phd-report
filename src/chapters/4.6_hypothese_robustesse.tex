...
\label{section:4.6-HYPOTHESE-ROBUSTESSE}
	...
	\label{section:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-INCOHERENCES}
		...
		\subsubsection{Protocole expérimental}
			...
			% Pseudo-code.
			Pour résumer le protocole expérimental que nous décrivons ci-dessous, vous pouvez vous référer au pseudo-code décrit dans \textsc{Algorithme~\ref{algorithm:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-INCOHERENCES-PROTOCOLE}}.
			
			\begin{algorithm}
				\KwData{jeu de données annoté (vérité terrain)}
				\KwIn{liste de taux de différences à insérer}
				%
				\ForEach{taux de différences à insérer}{
					...
					\Repeat{annotation de toutes les contraintes possibles}{
						...
						\textbf{échantillonnage de différences}: définir les contraintes qui seront inversées \;
						...
						\ElseIf{détection et correction des conflits d'annotation}{
							\textbf{intégration corrective}: ré-annoter les annotations en conflit \;
						}
						\textbf{clustering}: regrouper les données par similarité avec \texttt{clust.kmeans.cop} \;
						\textbf{évaluation}: estimer l'équivalence entre le \textit{clustering} obtenu et la vérité terrain \;
					}
				}
				\textbf{analyse}: déterminer l'impact par itération et par taux de différences de la correction \;
				%
				...
				\label{algorithm:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-INCOHERENCES-PROTOCOLE}
			\end{algorithm}
			...
			Puisque nous introduisons des différences d'annotations, des conflits vont apparaître dans le gestionnaire de contraintes.
		...
		\subsubsection{Discussion}
			...
			D'après l'analyse de la \textsc{Figure~\ref{figure:4.6.1-ETUDE-ROBUSTESSE-INTERETS-CORRECTION-INCOHERENCES}}, nous pouvons clairement déduire que l'absence de correction des incohérences pénalise la méthode et entraîne une dérive significative des résultats.
