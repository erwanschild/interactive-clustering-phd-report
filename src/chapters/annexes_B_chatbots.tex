...
\label{annex:B-ANNEXE-CHATBOTS}
		...
		\label{annex:B.2.2-ANNEXE-CHATBOTS-APPROCHES-CHAT-ORIENTED}
			...
			\begin{itemize}
				...
				\item[\faThumbsDown] Au sujet des inconvénients :
					...
					(\textit{besoin d'une quantité démesurée de données pour l'apprentissage, besoin de \texttt{GPU} et d'une infrastructure technique musclée, ...}) ;
					% Réponses erronées.
	...
	\label{annex:B.3-ANNEXE-CHATBOTS-DILEMME}
		...
		\begin{leftBarAuthorOpinion}
			...
			\end{itemize}
			De fait, les choix de conception de l'assistant reviennent à définir quels composants doivent être rigides (\textit{gestion de règles}), malléable (\textit{apprentissage supervisé}) ou totalement délégués à la machine (\textit{modèles de langage}).
		...
		% Exemples.
		\begin{leftBarExamples}
			Nous dressons une liste nous exhaustives d'approches hybrides :
			\begin{itemize}
