\chapter{Proposition d'un Clustering Interactif}
	...
	\label{section:3.2-DESCRIPTION-THEORIQUE}
		...
		\begin{algorithm}
			\KwIn{données non segmentées ; budget à disposition}
			%
			...
			\Repeat{segmentation satisfaisante OU budget épuisé}{
			    ...
			    \textbf{évaluation (1)}: estimer la pertinence et la stabilité de la segmentation \;
			    \textbf{évaluation (2)}: estimer le budget restant et les coûts restant à investir \;
			}
			%
			\KwResult{données segmentées (i.e. base d'apprentissage)}
			%
			\caption{\textit{
				Description en pseudo-code de la méthode d'annotation proposée employant le clustering interactif.
			}}
