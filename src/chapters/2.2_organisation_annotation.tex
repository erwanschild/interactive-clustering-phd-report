	...
	\label{section:2.2.1-ORGANISATION-ANNOTATION-ETAPES-CLES}
		...
		\label{section:2.2.1.A-ORGANISATION-ANNOTATION-ETAPES-CLES-MODELIZE-ANNOTATE}
			...
			\begin{leftBarInformation}
				...
				C'est généralement à ce stade que la théorie rencontre la pratique : certaines règles d'annotation peuvent difficilement être applicables, certaines données peuvent être ambiguës ou hors-sujet, deux annotateurs peuvent aussi avoir des avis différents sur l'annotation la plus adéquate, ...
		...
		\label{section:2.2.1.B-ORGANISATION-ANNOTATION-ETAPES-CLES-TRAIN-TEST}
			...
			Une métrique basique est l'\texttt{accuracy} (ou taux de bonnes prédictions), décrivant simplement le nombre de fois où le modèle a fait une bonne proposition sur l'ensemble du test.
		...
		\label{section:2.2.1.C-ORGANISATION-ANNOTATION-ETAPES-CLES-REVISE}
			...
			\begin{itemize}
				...
				\item un nouvel algorithme de \textit{Machine Learning} \textit{a priori} plus performant peut requérir une modélisation différente pour traiter le problème.
			\end{itemize}
			%
			\begin{leftBarExamples}
	...
	\label{section:2.2.2-ORGANISATION-ANNOTATION-ACTEURS}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\item des compétences de \texttt{gestion} pour s'assurer du respect des normes et du caractère privé ou confidentiel de certaines données.
			\end{itemize}
