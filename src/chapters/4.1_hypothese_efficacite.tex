	...
	\label{section:4.1.1-ETUDE-CONVERGENCE}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{enumerate}
				...
				paires de données \textbf{purement aléatoires} (noté \texttt{samp.random.full}), paires de données \textbf{pseudo-aléatoires} (sélectionnant des données provenant d'un même \textit{cluster} ; noté \texttt{samp.random.same}), paires de données \textbf{issues d'un même \textit{cluster} et étant les plus éloignées} (noté \texttt{samp.farthest.same}) et paires de données \textbf{issues de \textit{clusters} différents et étant les plus proches} (noté \texttt{samp.closest.diff}).
			...
			% Problème nombre de \textit{clusters}...
