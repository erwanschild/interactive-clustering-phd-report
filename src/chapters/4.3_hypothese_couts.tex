...
\label{section:4.3-HYPOTHESE-COUTS}
	...
	\label{section:4.3.1-ETUDE-COUTS-TEMPS-ANNOTATION}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				...
				\textbf{modélisation}: entraîner un modèle linéaire généralisé du temps d'annotation \;
				\textbf{simulation}: écrire l'équation du temps d'annotation d'un lot de contraintes \;
				%
				\KwResult{modélisation du temps d'annotation d'un lot de contraintes}
				%
				\caption{\textit{Description en pseudo-code du protocole expérimental de l'étude du temps d'annotation d'un lot de contraintes par un expert métier.}}
	...
	\label{section:4.3.2-ETUDE-COUTS-TEMPS-CALCUL}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				\KwData{jeux de données annotés (vérité terrain) de tailles différentes}
				...
				\ForEach{arrangement d'algorithmes et de paramètres à tester}{
					...
					\ElseIf {estimation de la tâche de \textbf{clustering}}{
						...
						\textbf{échantillonnage initial}: sélectionner une base de contraintes avec \texttt{samp.rand.full} \;
						\textbf{simulation d'annotation}: ajouter des contraintes en utilisant la vérité terrain \;
						\textbf{chronomètre}: \textbf{START} \;
				...
				\ForEach{algorithme à modéliser}{
					...
					\textbf{modélisation}: entraîner un modèle linéaire généralisé avec les facteurs retenus \;
					\textbf{simulation}: écrire l'équation du temps d'exécution avec des paramètres obtenus \;
				}
				%
				\KwResult{modélisation du temps d'exécution des différents algorithmes}
				%	
				\caption{\textit{Description en pseudo-code du protocole expérimental de l'étude du temps d'exécution des algorithmes du \textit{clustering} interactif.}}
	...
	\label{section:4.3.3-ETUDE-COUT-NOMBRE-CONTRAINTES}
		...
		\subsubsection{Protocole expérimental}
			...
			\begin{algorithm}
				...
				%	
				\caption{\textit{Description en pseudo-code du protocole expérimental de l'étude du nombre de contraintes nécessaires pour converger vers une vérité terrain pré-établie avec notre paramétrage favori du \textit{clustering} interactif.}}
	...
	\label{section:4.3.4-ETUDE-COUTS-TOTAL}
		...
		\subsection{Ouverture vers une annotation en parallèle du \textit{clustering}}
			...
			\begin{leftBarSummary}
				...
				\begin{itemize}
					\item L'annotation d'une contrainte nécessite en moyenne $8$ secondes : cette tâche est rapide et intuitive (cf. \textsc{Section~\ref{section:4.3.1-ETUDE-COUTS-TEMPS-ANNOTATION}}) ;
