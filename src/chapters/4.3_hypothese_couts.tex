...
\label{section:4.3-HYPOTHESE-COUTS}
	...
	\begin{tcolorbox}[
		...
		% Résumé des études.
		Afin de vérifier cette hypothèse, nous organiserons plusieurs expériences pour simuler ou déterminer ces durées : une étude du temps d'annotation par un expert métier (cf. sous-section~\ref{section:4.3.1-ETUDE-COUTS-TEMPS-ANNOTATION}), une étude du temps de calcul des algorithmes (cf. sous-section~\ref{section:4.3.2-ETUDE-COUTS-TEMPS-CALCUL}) et une étude du nombre de contraintes nécessaires (cf. sous-section~\ref{section:4.3.3-ETUDE-COUT-NOMBRE-CONTRAINTES}). Nous conclurons l'estimation du temps total d'un projet d'annotation en section~\ref{section:4.3.4-ETUDE-COUTS-TOTAL}.
	...
	\label{section:4.3.1-ETUDE-COUTS-TEMPS-ANNOTATION}
		...
		\subsubsection{Protocole expérimental}
			...
			\footnote{Nous pourrions faire le parallèle avec la composante \texttt{P600} communément admise en neuroscience pour caractériser la réaction provoquée par la dissonance grammaticale ou syntaxique d'une phrase (cf. CITATION). Nous arrondissons à $1$ seconde pour garder une marge d'erreur.
			}\todo{CITATION: Purves et al. (2008). Principles of Cognitive Neuroscience}
	...
	\label{section:4.3.4-ETUDE-COUTS-TOTAL}
		...
		\subsubsection{Discussion du coût total}
			...
			La mise en commun de ces résultats se retrouvent dans les équations~\ref{equation:4.3.4-ETUDE-COUT-UNE-ITERATION-SEQUENTIELLE}, \ref{equation:4.3.4-ETUDE-COUT-NOMBRE-ITERATIONS-SEQUENTIEL} et \ref{equation:4.3.4-ETUDE-COUT-TOTAL-SEQUENTIEL}.
			...
			Au final, pour un ensemble de $5~000$ données, il faut donc entre $10.4$ et $17.7$ jours de travail à un expert métier pour obtenir une base d'apprentissage avec une méthodologie basée sur le \textit{clustering} interactif.
			
			% Critique du résultat : comparaison avec une annotation classique, avoir plusieurs annotateurs, ...
			Pour critiquer l'approximation que nous avons faite lors de cette section, nous essayons de la comparer le temps nécessaire qu'il aurait fallu à un projet d'annotation traditionnelle, comme décrit en section (REFERENCE)\todo{ref: chapitre 3} et en figure~\ref{figure:4.3.4-ETUDE-COUT-TOTAL-ARCHITECTURE} (1).
			...
			$4$ jours de travail pour annoter $5~000$ données (à raison de $20$ secondes par données, estimation haute inspirée de (CITATION: Pradhan et al. (2007)) \todo{CITATION: Pradhan et al. (2007) SemEval-2007 Task 17: English Lexical Sample, SRL and All Words} où la "\textit{désambiguïsation du sens des mots}", présentée comme une tâche de classification, demande $17.5$ secondes par données) ;
			...
			Au total, nous estimons donc qu'il faut $7$ jours de travail à un expert métier pour obtenir une base d'apprentissage avec une méthodologie traditionnelle.
			En l'état, nous pouvons donc conclure que le \textit{clustering} interactif que nous proposons est en moyenne deux fois plus coûteux qu'une annotation manuelle classique ($10.4$ à $17.7$ jours vs $7$ jours), ce qui peut être un frein à son utilisation en situation réelle.
			...
			Cependant, une telle solution ne permet toujours pas d'être compétitif avec l'annotation traditionnelle si cette dernière dispose aussi de plusieurs opérateurs (\textit{avec $2$ annotateurs : de $8.5$ à $15.3$ jours vs $5$ jours ; avec $4$ annotateurs : de $6.8$ à $14.1$ jours vs $4$ jours}).
		...
		\subsection{Ouverture vers une annotation en parallèle du \textit{clustering}}
			...
			% Réalisation : Paralléliser annotation et clustering !
			...
			% Figure.
			Ces estimations mises à jour sont représentées sur la figure~\ref{figure:4.3.4-ETUDE-COUT-TOTAL} en fonction de plusieurs taille de jeu de données, et permettent de faire la comparaison avec la version séquentielle initialement présentée.
			...
			En reprenant une base de $5~000$ données et une marge de $2$ jours pour corriger et nommer les \textit{clusters}, cela représente $6.8$ jours de travail, un estimation équivalente à une annotation traditionnelle qui nécessite $7$ jours de travail d'après nos approximations.
			De plus, si nous ajoutons des plusieurs opérateurs, cette version parallèle reste compétitive (\textit{avec $2$ annotateurs : $4.5$ jours vs $5$ jours ; avec $4$ annotateurs : $3.3$ jours vs $4$ jours}).
