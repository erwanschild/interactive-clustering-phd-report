...
% : « \textit{combien dois-je investir ?} »
	...
	\label{section:4.3.1-ETUDE-COUTS-TEMPS-ANNOTATION}
		...
		\subsubsection{Protocole expérimental}
			...
			Pour ce faire, nous estimerons grossièrement le temps nécessaire à l'annotation réactive d'une contrainte, nous la comparerons au temps moyen estimé lors de notre expérience, et nous nous demanderons si la différence observée peut cacher un mécanisme cognitif plus complexe.
		...
		\paragraph{Analyse d'une session d'annotation de contraintes.}
			...
			En prenant en compte une marge d'erreur pour minimiser ce résultat, nous retiendrons $150$ contraintes.
		...
		\paragraph{Analyse des fonctionnalité de l'application d'annotation.}
			...
			\begin{leftBarAuthorOpinion}
				Ces différentes évolutions sont en cours d'analyse ou déjà intégrés dans l'application que nous proposons.
			\end{leftBarAuthorOpinion}
	...
	\label{section:4.3.2-ETUDE-COUTS-TEMPS-CALCUL}
		...
		\subsubsection{Discussion}
			...
			Nous maintenons donc les paramétrages obtenus pour ces tâches en section~\ref{section:4.2-HYPOTHESE-EFFICIENCE} sans analyses complémentaires, et nous utiliserons l'estimation temporelle du \textit{clustering} \texttt{clust.kmeans.cop} majorée de $10$\%.
			...
			\begin{leftBarSummary}
				...
				On estime le temps d'exécution de ce paramétrage avec l'équation suivante\footnote{Temps du paramétrage favori : environ $2.8$ minutes pour $1~000$ données ; environ $14.5$ minutes pour $5~000$ données.} :
				%
				\begin{equation}
					\label{equation:4.3.2-ETUDE-COUTS-TEMPS-CALCUL-PARAMETRAGE-FAVORI}
					\texttt{computation\_time}(\texttt{settings.favorite})~[s]~
					\propto~1.67 \cdot 10^{-1} \cdot \texttt{dataset\_size}
				\end{equation}
			\end{leftBarSummary}
	...
	\label{section:4.3.3-ETUDE-COUT-NOMBRE-CONTRAINTES}
		...
		\subsubsection{Discussion}
			...
			À première vue, une telle estimation peut être décourageante, car caractériser $15~750$ contraintes pour un jeu de $5~000$ données représente une lourde charge : environ $34$ heures d'annotation d'après l'équation~\ref{equation:4.3.1-ETUDE-COUT-COUTS-TEMPS-ANNOTATION} !
