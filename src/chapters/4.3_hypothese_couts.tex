...
% : « \textit{combien dois-je investir ?} »
	...
	\label{section:4.3.1-ETUDE-COUTS-TEMPS-ANNOTATION}
		...
		%%	\paragraph{Analyse du temps nécessaire :}
			...
			L'étude réalisée avec $14$ annotateurs sur des lots de $1~000$ contraintes a permis d'estimer à $202 + 6.92 \cdot \texttt{batch\_size}$ le temps nécessaire (en minutes) pour annoter un lot de contraintes.
	...
	\label{section:4.3.3-ETUDE-COUT-NOMBRE-CONTRAINTES}
		...
		\subsubsection{Résultats obtenus}
			...
			Le modèle linéaire généralisé entraîné sur les mesures du nombre de contraintes requis pour atteindre $90$\% de \texttt{v-measure} (\texttt{R²}: $> 0.999$, \texttt{llf}: $-4~324.2$, \texttt{llf\_null}: $-4~942.9$) nous permet de déduire l'équation suivante :
			%
			\begin{equation}
		...
		\subsubsection{Discussion}
			...
			On peut constater que la relation entre la taille du jeu de données et le nombre de contraintes à annoter est linéaire (pente de $3.05$) : doubler la taille d'un jeu de données doublera donc la charge de travail incombant à l'expert métier.
			...
			Ici, la différence de pente mesurée est de $0.14$ (\texttt{p-valeur}: $> 0.999$), soit un écart moyen d'environ $4.5$\% par rapport à la modélisation moyenne.
