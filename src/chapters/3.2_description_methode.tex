...
\label{section:3.2-DESCRIPTION-THEORIQUE}
	...
	\label{section:3.2.2-DESCRIPTION-THEORIQUE-DETAILLEE}
		...
		\begin{algorithm}
			...
			\textbf{interprétation}: trier et nommer les \textit{cluster} pour les exploiter \;
			%
		...
		La dernière étape consiste alors à \textbf{interpréter} ces \textit{cluster} pour pouvoir les exploiter (cf. \textsc{Algorithme~\ref{algorithm:3.2-CLUSTERING-INTERACTIF}}, \textit{ligne 13}).
		...
		\begin{leftBarExamples}
			...
			\begin{figure}[H]
				...
				\caption{
					...
					et \textbf{(2)} correspond à une première segmentation des données en $3$ \textit{cluster}.
					...
					et \textbf{(5)} correspond à la nouvelle segmentation des données en $3$ \textit{cluster} respectant les $6$ contraintes annotées.
