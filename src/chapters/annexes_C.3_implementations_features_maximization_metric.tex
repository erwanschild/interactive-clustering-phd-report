...
\label{annex:C.3-DESCRIPTION-IMPLEMENTATION-FEATURES-MAXIMIZATION-METRIC}
	...
	\label{annex:C.3.1-DESCRIPTION-IMPLEMENTATION-FEATURES-MAXIMIZATION-METRIC-CALCUL-FMEASURE}
		...
		% Description du calcul.
		La \textbf{\textit{Features Predominance}} d'une classe $c \in \mathcal{C}$ et d'une composante vectorielle $f \in \mathcal{F}$, notée $\texttt{FP}[f][c]$, est un score compris entre $0$ et $1$ qui est obtenu par le ratio entre :
		\begin{itemize}
			\item la somme des poids des vecteurs pour la composante $f$ et pour les données de la classe $c$, et
		...
		% Description du calcul.
		La \textbf{\textit{Features F-Measure}} d'une classe $c \in \mathcal{C}$ et d'une composante vectorielle $f \in \mathcal{F}$, notée $\texttt{FM}[f][c]$, est un score entre $0$ et $1$ qui est obtenu par la moyenne harmonique entre la \textit{Features Recall} et la \textit{Features Predominance}.
	...
	\label{annex:C.3.2-DESCRIPTION-IMPLEMENTATION-FEATURES-MAXIMIZATION-METRIC-SELECTION-FEATURES}
		...
		% Introduction.
		L'objectif de cette seconde étape est de supprimer les composantes vectorielle qui n'apportent pas d'information de manière générale.
		...
		% Description du calcul.
		La \textbf{\textit{Features Overall Average}}, notée $\overline{\overline{\texttt{FM}}}$, est un score entre $0$ et $1$ qui est obtenu par la moyenne de la \texttt{Features F-Measure} sur toutes les composantes et pour toutes les classes.
	...
	\label{annex:C.3.3-DESCRIPTION-IMPLEMENTATION-FEATURES-MAXIMIZATION-METRIC-ACTIVATION-FEATURES}
		...
		% Description du calcul.
		La \textbf{\textit{Features Marginal Average}} d'une composant sélectionnée $f \in \mathcal{F}_{\texttt{SELECTED}}$, notée $\overline{\texttt{FM}[f]}$, est un score entre $0$ et $1$ qui est obtenu par la moyenne locale de la \texttt{Features F-Measure} pour la composante $f$ et pour toutes les classes.
		...
		% Description du calcul.
		L'\textbf{activation de \textit{features}} se fait en comparant les valeurs de \texttt{Features F-Measures} à cette moyenne locale : si une composante sélectionnée $f \in \mathcal{F}_{\texttt{SELECTED}}$ a un score supérieur \texttt{FM}[f][c] à la moyenne locale $\overline{\texttt{FM}[f]}$ pour une classe $c \in \mathcal{C}$, alors cette composante $f$ est activée pour cette classe $c$.
	...
	%%%
	%%% Subsection C.3.4: Application à l'analyse à la classification de textes.
	%%%
